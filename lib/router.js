...
Router.prototype.remove = function( name ) {
  for (var i in this.routes) {
    if(this.routes[i].route_name == name) {
      //paraphrased array Remove from John Resig
      var rest = this.routes.slice(i+1 || this.length)
        this.routes.length = i
        this.routes.push.apply(this.routes,rest)
        i = i - 1
    }
  }
}
